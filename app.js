...
form.addEventListener("submit", function (e) {
  ...
  if (value <= 0 || value > 9) {
    //or if (isNaN(value) || value < 0 || value > 9) for parseInt(amount.value)
    result.innerHTML = `<p class="result">${text[random]}</p>`;
  } else {
    tempText = text.slice(0, value); //is an array from 0 index to value index

    // first solution
    result.innerHTML = "";
    tempText.forEach((txt) => {
      result.innerHTML += `<p class="result">${
        tempText[tempText.indexOf(txt)]
      }</p>`;
    });

    // second solution
    // let paragraph = tempText.map(myFunc).join("");
